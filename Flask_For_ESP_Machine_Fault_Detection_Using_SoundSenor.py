...
ALLOWED_NETWORK_PREFIX = '192.168.99'
...
def get_sensor_data():
    try:
        conn = mysql.connector.connect(**DATABASE_CONFIG)
        cursor = conn.cursor()
        cursor.execute("SELECT value FROM sensor_values ORDER BY timestamp DESC LIMIT 10")
        sensor_values = [row[0] for row in cursor.fetchall()]
    except mysql.connector.Error as err:
        print(err)
        sensor_values = []
    finally:
